...
export const importCommand = defineToolCommand(
	{
		...
		category: 'import',
		outputSchema: {
			processedFiles: { type: 'array', items: 'string', description: 'Successfully processed file paths' },
			skippedFiles: { type: 'array', items: 'SkippedFile', description: 'Skipped files with reasons' },
			totalTransactions: { type: 'number', description: 'Total transactions parsed' },
			duplicateTransactions: { type: 'number', description: 'Duplicate transactions skipped' },
			journalEntriesCreated: { type: 'number', description: 'Journal entries created' },
			transferPairsCreated: { type: 'number', description: 'Transfer pairs matched' },
			entryErrors: { type: 'number', description: 'Entries that failed to create' },
			accountsTouched: { type: 'array', items: 'string', description: 'Chart account IDs affected' },
			unmappedDescriptions: { type: 'array', items: 'string', description: 'Descriptions without mapping rules' },
			archivedFiles: { type: 'array', items: 'string', description: 'Files moved to archive' },
		},
		idempotent: false,
		rateLimit: null,
