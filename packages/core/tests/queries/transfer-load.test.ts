...
function insertNonTransfer(db: Database, journalId: string, postedAt: string, assetAccount: string, counterAccount: string, amountMinor: number): void {
	db.prepare(`INSERT INTO journal_entries (id, posted_at, posted_date, is_transfer, description) VALUES (?, ?, ?, 0, 'Payment')`).run(journalId, postedAt, postedAt.slice(0, 10));
...
function insertThreePostingEntry(db: Database, journalId: string, postedAt: string, account1: string, account2: string, account3: string): void {
	db.prepare(`INSERT INTO journal_entries (id, posted_at, posted_date, is_transfer, description) VALUES (?, ?, ?, 0, 'Split')`).run(journalId, postedAt, postedAt.slice(0, 10));
